...
Generates realistic test data for space station monitoring,
alien contacts, and crew missions.
"""
...
class SpaceStationGenerator:
    ...
    STATION_NAMES = [
        "International Space Station", "Lunar Gateway",
        "Mars Orbital Platform", "Europa Research Station",
        "Titan Mining Outpost", "Asteroid Belt Relay",
        "Deep Space Observatory", "Solar Wind Monitor",
        "Quantum Communications Hub"
    ]

    STATION_PREFIXES = [
        "ISS", "LGW", "MOP", "ERS", "TMO", "ABR", "DSO", "SWM", "QCH"
    ]
...
class CrewMissionGenerator:
    ...
    FIRST_NAMES = [
        "Sarah", "John", "Alice", "Michael", "Emma",
        "David", "Lisa", "Robert", "Maria", "James",
        "Anna", "William", "Elena", "Thomas", "Sofia", "Daniel"
    ]

    LAST_NAMES = [
        "Connor", "Smith", "Johnson", "Williams",
    ...
    def generate_crew_member(self, member_id: str) -> Dict[str, Any]:
        ...
        rank_bonus = {
            "cadet": 0, "officer": 2, "lieutenant": 5,
            "captain": 8, "commander": 12
        }
        years_experience = min(base_experience + rank_bonus[rank] + random.randint(-2, 3), 30)  # noqa: E501
    ...
    def generate_mission_data(self, count: int = 3) -> List[Dict[str, Any]]:
        ...
        for i in range(count):
            mission_id = (
                f"M{self.config.base_date.year}_"
                f"{random.choice(['MARS', 'LUNA', 'EUROPA', 'TITAN'])}"
            )
            destination = random.choice(self.DESTINATIONS)
            mission_name = (
                f"{destination} "
                f"{'Colony' if random.random() < 0.5 else 'Research'} Mission"
            )
            ...
            if duration > 365:
                ...
                experienced_count = (sum(1 for member in crew if member["years_experience"] >= 5))  # noqa: E501
...
def main() -> None:
    ...
    for station in stations:
        status = (
            "✅ Operational" if station["is_operational"] else "⚠️  Maintenance"
        )
    ...
    for contact in contacts:
        ...
        print(
            f"  {contact['contact_id']}: {contact['contact_type']} at "
            f"{contact['location']} - {verified}"
        )
    ...
    for mission in missions:
        ...
        print(
            f"    Crew: {len(mission['crew'])} members, "
